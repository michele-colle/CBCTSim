...
G4VPhysicalVolume *ICRP110PhantomConstruction::Construct()
{
  ...
  PlacePhantomInVolume(logicWorld);
  return fMotherVolume;
}


void ICRP110PhantomConstruction::PlacePhantomInVolume(G4LogicalVolume* logicVolume)
{
  // World Volume// Define Material Air
  G4double A;  // atomic mass
  G4double Z;  // atomic number
    ...
    for (G4int ix = 0; ix < fNVoxelX; ix++)
    {
      ...
      if (mateID_out < 0 || mateID_out >= nMaterials)
      {
        G4Exception("GmReadPhantomG4Geometry::ReadPhantomData",
                    ...
                    FatalException,
                    G4String("It should be between 0 and " + G4UIcommand::ConvertToString(nMaterials - 1) + ", while it is " + G4UIcommand::ConvertToString(OrgID)).c_str());
