...
TETModelImport::TETModelImport(G4String phantom, G4UIExecutive* ui)
{
	...
	else if(phantom == "MRCP-05F") phantomName = "MRCP-05F";
	else if(phantom == "MRCP-10M") phantomName = "MRCP-10M";
...
void TETModelImport::DataRead(G4String eleFile, G4String nodeFile)
{
	...
	// if (fs::exists((std::string)phantomDataPath) && fs::is_directory((std::string)phantomDataPath)) {
    // G4cout << "Listing files in directory: " << phantomDataPath << G4endl;
    // for (const auto& entry : fs::directory_iterator((std::string)phantomDataPath)) {
    //     G4cout << " - " << entry.path().filename() << G4endl;
    // }
	// } else {
	// 	G4cout << "Directory does not exist: " << phantomDataPath << G4endl;
	// }
